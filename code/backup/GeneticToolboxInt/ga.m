...
%                                %  by default
%
% Grupo de Control Predictivo y Optimizacin - CPOH
...
while (gaDat.gen<gaDat.MAXGEN),
    ...
    if isequal(round(old), round(gaDat.xmin))==1
        count = count + 1;
        if count >= 3
            break;
        end
...
% MUTATION ------------------------------------------------
Chrom = mutbga(SelCh,gaDat.FieldD,[gaDat.Pm 1]); % Codificacin Real.
...
delta(index)=aux2;
NewChrom=round(OldChrom+(MutMx.*range.*delta));
